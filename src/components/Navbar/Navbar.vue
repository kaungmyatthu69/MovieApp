    ...
    <div class="container mx-auto flex py-5 justify-between items-center px-7">
      ...
      <div class="flex gap-10 ">
        ...
        <div class="hidden md:block">
          <ul class="flex gap-5  ">
            <li class="relative group  cursor-pointer text-white " v-for="(item,index) in navList" :key="index">{{item.title}}
              <div class="absolute bg-white group-hover:block transition-all duration-300 hidden w-36  rounded shadow ">
                <div  v-for="(data,index) in item.list" :key="index" class="hover:bg-gray-100   text-black cursor-pointer px-3 py-2 " >
                  <p>{{data}}</p>
                </div>
              </div>
            </li>
      ...
      <div class="flex gap-5">
        <div class="relative">
          ...
          <span class="absolute inline-flex items-center w-4 h-4 justify-center rounded-full  text-xs bg-red-600  text-white -top-2 -right-1">4</span>
        </div>
...
const moviesData=ref(['Popular','Now Playing','Upcoming','Top related'])
